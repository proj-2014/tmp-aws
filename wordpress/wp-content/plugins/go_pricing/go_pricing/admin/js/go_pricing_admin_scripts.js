...
jQuery(document).ready(function($, undefined) {	
	...
	/ ---------------------------------------------------------------------- */
		...
		/* form ajax submit */
		$goPricingAdmin.find('#go-pricing-import-export-form').submit(function(){
			var $this=$(this);
			$.ajax({  
				type: 'post',
				url: ajaxurl,
				data: jQuery.param({ action: 'go_pricing_import_export_ajax_submit' })+'&'+$this.serialize(),
				beforeSend: function () {
					$goPricingAdmin.find('.ajax-loading').css('visibility','visible');
				}
			}).always(function() {
	...
	/ ---------------------------------------------------------------------- */
		...
		$goPricingAdmin.delegate('.go-pricing-thumbs a', 'click mouseenter mouseleave', function(e) {
			var $this=$(this);
		...
		$goPricingAdmin.find('#go-pricing-form').submit(function(){
			...
			$.ajax({  
				...
				$('select').not('[class*="go-pricing-sc-selector"]').trigger('change');
